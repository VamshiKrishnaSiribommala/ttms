...
namespace TrainWorkingApp
{
    public class AuthService
    {
        ...
        #region User Management (Admin & Registration)
        ...
        public bool RegisterUser(string fullName, DateTime? dob, string gender, string email, string phone,
            string address, string department, string course, string yearOfJoining, string collegeOrOrg,
            string username, string password, out string errorMessage)
        {
            return CreateUser(fullName, dob, gender, email, phone, address, department, course, yearOfJoining, collegeOrOrg, username, password, password, "Active", out errorMessage);
        }
        ...
        #region Password Recovery & Logs

        public bool ResetPassword(string username, string phone, DateTime dob, string newPassword, out string errorMessage)
        {
            return ResetPasswordByVerification(username, phone, dob, newPassword, newPassword, out errorMessage);
        }
            ...
            out string errorMessage)
        {
            ...
            if (string.IsNullOrWhiteSpace(username)) { errorMessage = "Please enter your Username / User ID."; return false; }
            if (string.IsNullOrWhiteSpace(phone)) { errorMessage = "Please enter your Registered Mobile Number."; return false; }
            if (string.IsNullOrWhiteSpace(newPassword)) { errorMessage = "Please enter a New Password."; return false; }
            if (newPassword.Length < 4) { errorMessage = "New Password must be at least 4 characters."; return false; }
            if (newPassword != confirmPassword) { errorMessage = "Passwords do not match."; return false; }
            ...
            try
            {
                    ...
                    WHERE Username = @u"))
                {
                    ...
                    bool phoneMatches = cleanInputPhone == cleanStoredPhone;
                    if (cleanInputPhone.Length >= 10 && cleanStoredPhone.Length >= 10)
                    {
                        phoneMatches = cleanInputPhone.EndsWith(cleanStoredPhone) || cleanStoredPhone.EndsWith(cleanInputPhone);
                    }

                    if (!phoneMatches && !string.IsNullOrWhiteSpace(storedPhone))
                    {
        ...
        public DataTable GetPasswordResetLogs(string searchKeyword = "")
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(searchKeyword))
                {
                    sql += " WHERE Username LIKE @kw OR FullName LIKE @kw OR Phone LIKE @kw OR Status LIKE @kw ";
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine("Error getting reset logs: " + ex.Message);
                return new DataTable();
            }
        }
