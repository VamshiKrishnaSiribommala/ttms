...
namespace TrainWorkingApp
{
    public class Form_Auth007_CommonStarter : Form
    {
        ...
        {
            this.Text = "AUTHORITY FOR COMMON STARTER SIGNAL (T/512) - AUTH-007";
            ...
            { 
                Text = "AUTHORITY FOR COMMON STARTER SIGNAL (T/512)",
            ...
            y += 45;
            AddLabel("Common Starter Signal ID *:", 30, y);
            txtSignalID = new TextBox { Location = new Point(290, y), Size = new Size(250, 26) };
            this.Controls.Add(txtSignalID);
            y += 45;
            ...
            y += 45;
            chkRouteSetting = new CheckBox { Text = "Route setting confirmed and starter signal lowered *", Location = new Point(290, y), Size = new Size(560, 24), Checked = false };
            this.Controls.Add(chkRouteSetting);
            y += 30;
            chkBlockSection = new CheckBox { Text = "Block section status verified clear *", Location = new Point(290, y), Size = new Size(560, 24), Checked = false };
            ...
            btnView.Click += (s, e) => new ViewRecordsForm("Authority_Common_Starter", "Common Starter Signal Authorities").ShowDialog(this);
        ...
        {
            txtRefNo.Text = "AUTH-CS-" + DateTime.Now.ToString("yyyyMMdd") + "-" + new Random().Next(100, 999);
        }
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            var vals = new Dictionary<string, object>
            {
                ...
                { "Starter_Signal_ID", txtSignalID.Text.Trim() },
                { "Starting_Station", txtStation.Text.Trim() },
                { "Issuing_Officer", txtOfficer.Text.Trim() },
                { "Route_Setting", chkRouteSetting.Checked },
                { "Block_Section_Status", chkBlockSection.Checked },
                { "Loco_Pilot_Ack", chkLocoAck.Checked }
            };

            if (!ValidationHelper.ValidateForm("007", vals)) return;

            try
            {
                using (SqlCommand cmd = new SqlCommand(@"INSERT INTO Authority_Common_Starter 
                    (Authority_Reference_No, Train_No, Line_No, Starter_Signal_ID, Starting_Station, Date_Time_of_Issue, Issuing_Officer, Route_Setting, Block_Section_Status, Loco_Pilot_Ack)
                    VALUES (@ref, @train, @line, @sig, @stn, @dt, @off, @route, @blk, @ack)"))
                {
                    ...
                    cmd.Parameters.AddWithValue("@sig", txtSignalID.Text.Trim());
                    ...
                    cmd.Parameters.AddWithValue("@route", chkRouteSetting.Checked);
