...
namespace TrainWorkingApp
{
    ...
    /// Railway Train Working Authorities via WhatsApp, Telegram, or Both in 1-Click.
    /// </summary>
    public static class RecordDispatchService
    {
        ...
        public static List<DispatchContact> LoadSavedContacts()
        {
            ...
            // Default Railway Operational Contacts on initial run
        ...
        #region Form Handler (Invoked when SEND button is clicked on any authority form)
        ...
        public static void HandleSendButtonClick(Form form)
        {
            ...
            if (btnSave == null)
            {
                MessageBox.Show("Could not find the Save action for this authority form.", "Dispatch Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // 2. Register / Authority details
            string regTitle = form.Text;
            string regCode = "AUTH";
            Match m = Regex.Match(regTitle, @"AUTH[-_ ]?([0-9]{3})", RegexOptions.IgnoreCase);
            if (m.Success)
            {
                regCode = "AUTH-" + m.Groups[1].Value;
            }
            else
            {
                Match m2 = Regex.Match(form.GetType().Name, @"Auth([0-9]{3})", RegexOptions.IgnoreCase);
                if (m2.Success) regCode = "AUTH-" + m2.Groups[1].Value;
            }
            ...
            string logId = "TW-AUTH-" + DateTime.Now.ToString("yyyyMMdd-HHmmss");
            foreach (var kvp in fieldData)
            {
                if (kvp.Key.ToLower().Contains("ref") || kvp.Key.ToLower().Contains("log") || kvp.Key.ToLower().Contains("id") || kvp.Key.ToLower().Contains("number") || kvp.Key.ToLower().Contains("order"))
                {
                    if (!string.IsNullOrWhiteSpace(kvp.Value) && (kvp.Value.StartsWith("TW-") || kvp.Value.StartsWith("AUTH-") || kvp.Value.StartsWith("T/") || kvp.Value.StartsWith("TFC")))
                    {
                        logId = kvp.Value;
                        break;
                    }
        ...
        private static bool ValidateFormBeforeDispatch(Form form, string regCode, Dictionary<string, string> fieldData, out string errorMessage)
        {
            ...
            // Check 3: Explicit inspection of reflection controls
            ...
            foreach (var f in formFields)
            {
                ...
                else if (name.Contains("signalno") || name.Contains("signal_no"))
                {
                    if (f.GetValue(form) is TextBox tb && string.IsNullOrWhiteSpace(tb.Text) && !missing.Any(m => m.ToLower().Contains("signal")))
                        missing.Add("Signal Number");
                }
                else if (name.Contains("officer"))
                {
                    if (f.GetValue(form) is TextBox tb && string.IsNullOrWhiteSpace(tb.Text) && !missing.Any(m => m.ToLower().Contains("officer")))
                        missing.Add("Issuing Officer");
                }
                else if (name.Contains("reason"))
                {
                    if (f.GetValue(form) is ComboBox cb && (cb.SelectedIndex < 0 || string.IsNullOrWhiteSpace(cb.Text)) && !missing.Any(m => m.ToLower().Contains("reason")))
                        missing.Add("Reason");
                }
            }

            // Check 4: TrainWorkingValidationEngine evaluation
            ...
            foreach (var f in formFields)
            {
                if (typeof(Control).IsAssignableFrom(f.FieldType))
                {
                    ...
                    if (c != null)
                    {
                        ...
                        string baseName = raw;
                        if (baseName.StartsWith("txt") || baseName.StartsWith("cmb") || baseName.StartsWith("dtp") || baseName.StartsWith("chk") || baseName.StartsWith("num"))
                        ...
                        if (val != null)
                        {
                            ...
                            {
                                engineValues["Train_No"] = val;
                                engineValues["TrainNo"] = val;
                                engineValues["TrainNumber"] = val;
                            }
                            if (lower.Contains("signal") && !lower.Contains("loc"))
                            {
                                engineValues["Signal_No"] = val;
                                engineValues["SignalNo"] = val;
                                engineValues["SignalNumber"] = val;
                            }
                            if (lower.Contains("loc") || lower.Contains("section"))
                            {
                                engineValues["Signal_Location"] = val;
                                engineValues["Section_Location"] = val;
                                engineValues["Location"] = val;
                            }
                            if (lower.Contains("station"))
                            {
                                engineValues["Issuing_Station"] = val;
                                engineValues["Station"] = val;
                            }
                            if (lower.Contains("officer") || lower.Contains("staff"))
                            {
                                engineValues["Issuing_Officer"] = val;
                                engineValues["Officer"] = val;
                            }
                            if (lower.Contains("speed"))
                            {
                                engineValues["Permitted_Speed"] = val;
                                engineValues["Speed_Restriction"] = val;
                                engineValues["Speed"] = val;
                            }
                            if (lower.Contains("route"))
                            {
                                engineValues["Route_Clearance"] = val;
                                engineValues["RouteClearance"] = val;
                            }
                            if (lower.Contains("protection") || lower.Contains("lineprotect"))
                            {
                                engineValues["Line_Protection"] = val;
                                engineValues["LineProtection"] = val;
                            }
                            if (lower.Contains("block"))
                            {
                                engineValues["Block_Section"] = val;
                                engineValues["BlockSection"] = val;
                            }
                            if (lower.Contains("ack"))
                            {
                                engineValues["Loco_Ack"] = val;
                                engineValues["LocoAck"] = val;
                            }
                        }
                    }
                }
            ...
            ValidationResult valResult = TrainWorkingValidationEngine.Validate(regCode, engineValues);
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendLine("❌ Cannot Send Authority Memo: Form is Incomplete!\n");
                sb.AppendLine("Please fill in all mandatory fields marked with (*) and confirm all safety verifications before sending:\n");
        ...
        public static Dictionary<string, string> ExtractFormFields(Form form)
        {
            ...
            try
            {
                ...
                HashSet<Control> mappedInputs = new HashSet<Control>();

                // Extract values from all input controls via reflection
                foreach (var field in formFields)
                {
                    if (typeof(Control).IsAssignableFrom(field.FieldType))
                    {
                        ...
                        if (c != null && (c is TextBox || c is ComboBox || c is DateTimePicker || c is CheckBox || c is RichTextBox || c is NumericUpDown))
                        {
                            ...
                            if (c is CheckBox chkBox && !string.IsNullOrWhiteSpace(chkBox.Text))
                            {
                                displayName = chkBox.Text.Trim().TrimEnd('*', ':', ' ');
                            }
                            else
                            {
                ...
                // Spatial proximity fallback for remaining labels (unmapped controls only)
                ...
                {
                    string fieldName = lbl.Text.Trim().TrimEnd('*', ':', ' ');
                    if (string.IsNullOrWhiteSpace(fieldName) || fieldName.Length < 3) continue;
                    ...
                    foreach (var inp in allInputs)
                    {
                        if (inp is CheckBox || mappedInputs.Contains(inp)) continue; // Checkboxes already carry their own full text; already mapped controls don't need another label

                        Point inpPt = inp.PointToScreen(Point.Empty);
                        int dy = Math.Abs(inpPt.Y - lblPt.Y);
                        int dx = inpPt.X - lblPt.X;

                        if (dy < 35 && dx >= -10 && dx < 350)
                        {
                            int dist = dy * 10 + dx;
                            if (dist < minDistance)
                            {
            ...
            if (!data.ContainsKey("Logged At") && !data.ContainsKey("Date & Time of Issue"))
            {
                data["Logged At"] = DateTime.Now.ToString("dd-MMM-yyyy hh:mm:ss tt");
            }
            if (!data.ContainsKey("Duty Staff / Officer") && !data.ContainsKey("Issuing Officer"))
            {
                data["Duty Staff / Officer"] = $"{SessionManager.CurrentFullName} ({SessionManager.CurrentDepartment})";
            }
        ...
        private static string NormalizeFieldName(string name)
        {
            ...
            string s = Regex.Replace(name, @"\s*\(.*?\)", "").Replace(" ", "").Replace("/", "").Replace("-", "").Replace("&", "").Replace("_", "").ToLower();
            if (s == "trainno") return "trainnumber";
            if (s == "signalno") return "signalnumber";
            if (s == "location") return "signallocation";
            if (s == "officer") return "issuingofficer";
            if (s == "station") return "issuingstation";
            if (s == "speed") return "permittedspeed";
            if (s == "refno" || s == "authorityreferencenosystem" || s == "authorityrefno") return "authorityrefno";
            if (s == "issue") return "datetimeofissue";
            return s;
        }
        ...
        private static void CollectControls(Control parent, List<Label> labels, List<Control> inputs)
        {
            ...
            {
                // Skip header banners, navigation bars, title bars, and buttons
                string pName = c.Name?.ToLower() ?? "";
                if (pName.Contains("header") || pName.Contains("topheader") || pName.Contains("bottomaction") || 
                    pName.Contains("banner") || pName.Contains("title") || pName.Contains("user") || 
                    pName.Contains("nav") || pName.Contains("bar") || c is Button)
                {
                    continue;
                }

                // Also skip docked top/bottom panels or banners positioned at the top
                if (c is Panel p && (p.Dock == DockStyle.Top || p.Dock == DockStyle.Bottom || (p.Top <= 0 && p.Height <= 85)))
                {
                    continue;
                }
                ...
                if (c is Label lbl)
                {
                    string lName = lbl.Name?.ToLower() ?? "";
                    string lText = lbl.Text?.Trim() ?? "";

                    if (lText.Length > 2 &&
                        !lName.StartsWith("lbllogo") && !lName.StartsWith("lblpath") && !lName.StartsWith("lbluser") && 
                        !lName.StartsWith("lbltitle") && !lName.StartsWith("lblsub") && !lName.StartsWith("lblavatar") &&
                        !lText.ToUpper().Contains("REGISTER") && !lText.ToUpper().Contains("TRAIN MANAGEMENT") && !lText.ToUpper().Contains("TRAIN WORKING") &&
                        !lText.ToUpper().Contains("STATION OPERATOR") && !lText.ToUpper().Contains("MY PROFILE") && !lText.ToUpper().Contains("BACK TO HUB") &&
                        !lText.ToUpper().Contains("CLOSE") && !lText.ToUpper().Contains("OPERATIONAL AUTHORITIES") && !lText.ToUpper().Contains("AUTHORITY TO PASS") &&
                        !lText.StartsWith("👤") && !lText.StartsWith("🚆"))
                    {
                        labels.Add(lbl);
                    }
                }
        ...
        private static string GetControlValue(Control c)
        {
            ...
            if (c is CheckBox chk) return chk.Checked ? "YES (Verified Clear)" : "NO (Pending)";
        ...
        public static string GenerateRecordPdfMemo(string regCode, string regTitle, string logId, Dictionary<string, string> fields)
        {
            try
            {
                ...
                string fileName = $"TW_Dispatch_{safeCode}_{DateTime.Now:yyyyMMdd_HHmmss}.pdf";
                ...
                using (StreamWriter sw = new StreamWriter(fs, Encoding.ASCII))
                {
                    ...
                    sb.AppendLine("(INDIAN RAILWAYS - TRAIN WORKING SYSTEM) Tj");
                    sb.AppendLine("1 0 0 1 36 770 Tm");
                    sb.AppendLine("/F1 12 Tf");
                    sb.AppendLine($"({EscapePdfText($"OFFICIAL AUTHORITY MEMO  |  {regTitle.ToUpper()}")}) Tj");
                    sb.AppendLine("1 0 0 1 36 755 Tm");
                    sb.AppendLine("/F2 9 Tf");
                    sb.AppendLine($"({EscapePdfText($"Train Working Operational Authority  |  Ref: {logId}")}) Tj");
                    sb.AppendLine("ET");
                    ...
                    sb.AppendLine($"({EscapePdfText($"Authority Ref ID: {logId}")}) Tj");
                    ...
                    sb.AppendLine($"({EscapePdfText($"Authority Code: {regCode}")}) Tj");
                    ...
                    sb.AppendLine("(RECORDED VALUE / AUTHORITY DETAILS) Tj");
                    sb.AppendLine("ET");
                    ...
                    sb.AppendLine("(OFFICIAL RAILWAY DIGITAL RECORD - VERIFIED & DISPATCHED VIA TRAIN WORKING SYSTEM) Tj");
        ...
        private static string EscapePdfText(string input)
        {
            ...
            {
                if (ch >= 32 && ch <= 126) ascii.Append(ch);
                else if (ch == '\t') ascii.Append("  ");
                else ascii.Append(' ');
            }
        ...
        public static string UploadPdfForDirectLink(string pdfPath)
        {
            ...
            // Provider 1: Direct Raw PDF Stream
            try
            {
                using (var client = new System.Net.Http.HttpClient())
                {
                    client.Timeout = TimeSpan.FromSeconds(6);
                    client.DefaultRequestHeaders.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) TrainWorkingApp/1.0");
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("🚆 *INDIAN RAILWAYS — TRAIN WORKING AUTHORITY MEMO*");
            sb.AppendLine("━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            sb.AppendLine($"📋 *Authority Register:* {regTitle} ({regCode})");
            sb.AppendLine($"🆔 *Authority Ref No:* `{logId}`");
            ...
            sb.AppendLine("📝 *AUTHORITY DETAILS:*");

            foreach (var kvp in fields)
            {
                if (kvp.Key == "Logged At" || kvp.Key == "Duty Staff / Officer") continue;
            ...
            sb.AppendLine("━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            sb.AppendLine("✅ _Sent via Train Working Management System 1-Click Operational Dispatch._");
        ...
        public static bool DispatchToTelegram(string telegramHandle, string messageText, string pdfPath)
        {
            try
            {
                CopyPdfFileToClipboard(pdfPath);

                string cleanHandle = telegramHandle.Trim().TrimStart('@');
                string encodedMsg = Uri.EscapeDataString(messageText);

                string url;
                if (!string.IsNullOrWhiteSpace(cleanHandle))
                {
                    url = $"https://t.me/{cleanHandle}?text={encodedMsg}";
                }
                else
                {
                    url = $"https://t.me/share/url?url={Uri.EscapeDataString(pdfPath)}&text={encodedMsg}";
                }
        ...
        public static void OpenPdfFile(string pdfPath)
        {
            try
            {
                ...
                {
                    MessageBox.Show("PDF file does not exist: " + pdfPath, "File Not Found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void OpenDispatchesFolder()
        {
            try
            {
                if (!Directory.Exists(DispatchesFolder))
                ...
                Process.Start(new ProcessStartInfo
                {
                    FileName = DispatchesFolder,
                    UseShellExecute = true
                });
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error opening Dispatches folder: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
